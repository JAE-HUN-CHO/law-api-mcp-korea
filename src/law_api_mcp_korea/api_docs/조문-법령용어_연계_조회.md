...
## 3. 요청 URL (Endpoint)
- https://www.law.go.kr/DRF/lawSearch.do?target=joRltLstrm

## 4. 요청 변수 (Request Parameters) 명세표
| 변수명 | 타입/필수 여부 | 설명 |
| --- | --- | --- |
| OC | string, 필수 | 사용자 이메일 ID |
| target | string : joRltLstrm(필수) | 서비스 대상 |
...
- 예시 1 (XML)
```text
https://www.law.go.kr/DRF/lawSearch.do?target=joRltLstrm&OC=test&type=XML&query=개인정보&display=20&page=1
```
- 예시 2 (JSON)
```text
https://www.law.go.kr/DRF/lawSearch.do?target=joRltLstrm&OC=test&type=JSON&query=개인정보&display=20&page=1
```
#### 응답 예시
- XML 구조 예시
```xml
