...
## 1. API 명칭 및 기본 설명
- **API 명칭**: 신구법 목록 조회
- **기본 설명**: 신구법 비교 목록을 조회하는 API

## 2. 가이드 페이지 URL
- https://open.law.go.kr/LSO/openApi/guideResult.do?htmlName=oldAndNewListGuide

## 3. 요청 URL (Endpoint)
- http://www.law.go.kr/DRF/lawSearch.do?target=oldAndNew
...
- 예시 1 (XML)
```text
http://www.law.go.kr/DRF/lawSearch.do?target=oldAndNew&OC=test&type=XML&search=1&query=자동차관리법&display=20&page=1
```
- 예시 2 (JSON)
```text
http://www.law.go.kr/DRF/lawSearch.do?target=oldAndNew&OC=test&type=JSON&search=1&query=자동차관리법&display=20&page=1
```
#### 응답 예시
- XML 구조 예시
```xml
