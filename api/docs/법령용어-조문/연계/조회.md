...
## 1. API 명칭 및 기본 설명
- **API 명칭**: 법령용어-조문 연계 조회
- **기본 설명**: 법령용어-조문 연계 정보를 조회하는 API

## 2. 가이드 페이지 URL
- https://open.law.go.kr/LSO/openApi/guideResult.do?htmlName=lstrmRltJoGuide

## 3. 요청 URL (Endpoint)
- https://www.law.go.kr/DRF/lawSearch.do?target=lstrmRltJo

## 4. 요청 변수 (Request Parameters) 명세표
| 변수명 | 타입/필수 여부 | 설명 |
| --- | --- | --- |
| OC | string, 필수 | 사용자 이메일 ID |
| target | string : lstrmRltJo(필수) | 서비스 대상 |
...
- 예시 1 (XML)
```text
https://www.law.go.kr/DRF/lawSearch.do?target=lstrmRltJo&OC=test&type=XML&query=자동차관리법&display=20&page=1
```
- 예시 2 (JSON)
```text
https://www.law.go.kr/DRF/lawSearch.do?target=lstrmRltJo&OC=test&type=JSON&query=자동차관리법&display=20&page=1
```
#### 응답 예시
- XML 구조 예시
```xml
