...
| nw | int | 1: 현행, 2: 연혁(기본: 현행) |
| search | int | 검색범위(기본: 1 행정규칙명, 2 본문검색) |
| query | string | 검색어 |
| display | int | 검색된 결과 개수(default=20, max=100) |
| page | int | 검색 결과 페이지(default=1) |
| org | string | 소관부처 코드 |
| knd | string | 행정규칙 종류별 검색(훈령/예규/고시/공고/지침/기타) |
| gana | string | 사전식 검색 |
| sort | string | 정렬옵션(lasc/ldes/dasc/ddes/nasc/ndes/efasc/efdes) |
| date | int | 행정규칙 발령일자 |
...
- 예시 1 (XML)
```text
http://www.law.go.kr/DRF/lawSearch.do?target=admrul&OC=test&type=XML&search=1&query=학교&display=20&page=1
```
- 예시 2 (JSON)
```text
http://www.law.go.kr/DRF/lawSearch.do?target=admrul&OC=test&type=JSON&search=1&query=학교&display=20&page=1
```
#### 응답 예시
- XML 구조 예시
```xml
